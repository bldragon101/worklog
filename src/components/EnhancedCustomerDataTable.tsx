...
export function EnhancedCustomerDataTable({ 
  data, 
  isLoading, 
  onEdit, 
  onDelete, 
  ...
  onAddCustomer,
  filters 
}: EnhancedCustomerDataTableProps) {
            ...
            <div className="flex items-center justify-between gap-2">
              <SheetTitle className="truncate text-left">
                {isLoading && !selectedRow ? (
                  ...
                  `Customer Details - ${selectedRow?.original.customer}`
                )}
              </SheetTitle>
            ...
            {selectedRow && (
              <dl className="divide-y">
                ...
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  ...
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.contact}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Tray</dt>
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.tray ? `$${selectedRow.original.tray.toLocaleString()}` : "N/A"}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Crane</dt>
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.crane ? `$${selectedRow.original.crane.toLocaleString()}` : "N/A"}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Semi</dt>
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.semi ? `$${selectedRow.original.semi.toLocaleString()}` : "N/A"}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Semi Crane</dt>
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.semiCrane ? `$${selectedRow.original.semiCrane.toLocaleString()}` : "N/A"}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Fuel Levy</dt>
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.fuelLevy ? `${selectedRow.original.fuelLevy}%` : "N/A"}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Tolls</dt>
                  <dd className="font-mono w-full text-right">
                    {selectedRow.original.tolls ? "Yes" : "No"}
                  </dd>
                </div>
                ...
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  ...
                  <dd className="font-mono w-full text-right">
                    {new Date(selectedRow.original.createdAt).toLocaleDateString()}
                  </dd>
                </div>
                <div className="flex gap-4 my-1 py-1 text-sm justify-between items-center w-full">
                  <dt className="shrink-0 text-muted-foreground">Updated</dt>
                  <dd className="font-mono w-full text-right">
                    {new Date(selectedRow.original.updatedAt).toLocaleDateString()}
                  </dd>
                </div>
              </dl>
            )}
