...
export function InlineCellSelect({
  ...
  return (
    <Popover open={open} onOpenChange={(isOpen) => handleOpenChange({ isOpen })}>
      <PopoverTrigger asChild>
        <button
          ...
          onKeyUp={(e) => { if (e.key === "Enter" || e.key === " ") handleOpenChange({ isOpen: true }); }}
          onFocus={onFocus}
        >
        ...
        >
          {loading ? (
            ...
            <div className="p-2 text-xs text-muted-foreground">
              {searchQuery ? "No matches. Press Enter to use typed value." : "No options."}
            </div>
          ) : (
            filteredOptions.map((option, index) => (
              ...
              >
                <Check
                  className={cn("h-3 w-3 shrink-0", value === option ? "opacity-100" : "opacity-0")}
                />
