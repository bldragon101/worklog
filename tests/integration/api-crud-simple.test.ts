...
async function makeRequest(method: string, path: string, body?: any) {
  ...
  // Import handlers dynamically to avoid module loading issues
  if (path.startsWith('/api/jobs/') && path !== '/api/jobs') {
    const { GET, PUT, DELETE } = await import('@/app/api/jobs/[id]/route');
...
describe('API Integration Tests - HTTP Style', () => {
  ...
  describe('Jobs API', () => {
    ...
    test('should update job by ID', async () => {
      ...
      const response = await makeRequest('PUT', `/api/jobs/${job.id}`, updateData);
      const data = await response.json();
    ...
    test('should delete job by ID (admin user)', async () => {
      ...
      const deletedJob = await prisma.jobs.findUnique({ where: { id: job.id } });
  ...
  describe('Customers API', () => {
    test('should create a new customer with valid data', async () => {
      const response = await makeRequest('POST', '/api/customers', validCustomerData);
      const data = await response.json();
