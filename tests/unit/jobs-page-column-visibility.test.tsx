...
jest.mock('@/components/ui/table-loading-skeleton', () => ({
  TableLoadingSkeleton: ({ rows, columns }: { rows: number; columns: number }) => (
...
jest.mock('@/components/ui/progress-dialog', () => ({
  ProgressDialog: () => <div data-testid="progress-dialog">Progress Dialog</div>,
}));

jest.mock('@/components/ui/job-attachment-upload', () => ({
  JobAttachmentUpload: () => <div data-testid="job-attachment-upload">Job Attachment Upload</div>,
}));
...
describe('Jobs Page Column Visibility', () => {
  ...
  it('shows loading skeleton when jobs are loading', async () => {
    ...
    expect(screen.getByText('Loading skeleton with 10 rows and 12 columns')).toBeInTheDocument();
  ...
  it('initializes with undefined column visibility to allow meta.hidden to work', async () => {
    ...
    const visibilityState = screen.getByTestId('column-visibility-state').textContent;
  ...
  it('allows toggling column visibility for runsheet column', async () => {
    ...
    await waitFor(() => {
      const visibilityState = JSON.parse(screen.getByTestId('column-visibility-state').textContent || '{}');
...
describe('Job Columns Meta Configuration', () => {
  it('should have meta.hidden = true for specified columns', () => {
    ...
    const hiddenColumns = ['runsheet', 'invoiced', 'driverCharge', 'eastlink', 'citylink'];
    
    hiddenColumns.forEach(columnKey => {
      const column = columns.find(col => 
        'accessorKey' in col && col.accessorKey === columnKey
      );
  ...
  it('should not have meta.hidden = true for visible columns', () => {
    ...
    const visibleColumns = ['date', 'driver', 'customer', 'startTime', 'finishTime', 'status'];
