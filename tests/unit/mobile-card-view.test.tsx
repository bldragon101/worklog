...
describe("MobileCardView", () => {
  describe("Basic Rendering", () => {
    ...
    it("renders loading skeleton when loading", () => {
      const fields = [{ key: "date", label: "Date", isTitle: true }];

      const { container } = render(
        <MobileCardView data={[]} fields={fields} isLoading={true} />,
      );
  ...
  describe("Checkbox Functionality", () => {
    ...
    it("renders checkboxes with correct states", () => {
      ...
      const firstJobRunsheet = checkboxes.find(
        (cb) => cb.getAttribute("id") === "1-runsheet",
      );
      const firstJobInvoiced = checkboxes.find(
        (cb) => cb.getAttribute("id") === "1-invoiced",
      );
  ...
  describe("Loading States", () => {
    it("shows loading skeleton with correct number of items", () => {
      const { container } = render(
        <MobileCardView data={[]} fields={[]} isLoading={true} />,
      );
