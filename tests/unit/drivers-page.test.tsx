...
describe("Drivers Page Logic", () => {
  describe("Driver data processing", () => {
    ...
    it("should filter drivers by type", () => {
      ...
      const subcontractors = mockDriverData.filter(
  ...
  describe("Driver rate management", () => {
    it("should handle tray rates", () => {
      const driversWithTrayRates = mockDriverData.filter(
        (d) => d.tray !== null,
      );
      expect(driversWithTrayRates).toHaveLength(2);

      const totalTrayRates = mockDriverData.reduce(
        (sum, driver) => sum + (driver.tray || 0),
        0,
      );
    ...
    it("should handle crane rates", () => {
      const driversWithCraneRates = mockDriverData.filter(
        (d) => d.crane !== null,
      );
      expect(driversWithCraneRates).toHaveLength(2);

      const totalCraneRates = mockDriverData.reduce(
        (sum, driver) => sum + (driver.crane || 0),
        0,
      );
    ...
    it("should handle semi rates", () => {
      const driversWithSemiRates = mockDriverData.filter(
        (d) => d.semi !== null,
      );
      expect(driversWithSemiRates).toHaveLength(2);

      const totalSemiRates = mockDriverData.reduce(
        (sum, driver) => sum + (driver.semi || 0),
        0,
      );
    ...
    it("should handle semi crane rates", () => {
      const driversWithSemiCraneRates = mockDriverData.filter(
        (d) => d.semiCrane !== null,
      );
      expect(driversWithSemiCraneRates).toHaveLength(2);

      const totalSemiCraneRates = mockDriverData.reduce(
        (sum, driver) => sum + (driver.semiCrane || 0),
        0,
      );
    ...
    it("should handle break deductions", () => {
      const totalBreaks = mockDriverData.reduce(
        (sum, driver) => sum + (driver.breaks || 0),
        0,
      );
  ...
  describe("Driver type-specific features", () => {
    it("should identify drivers with tolls enabled", () => {
      ...
      const driversWithoutTolls = mockDriverData.filter(
        (d) => d.tolls === false,
      );
    ...
    it("should handle fuel levy for subcontractors", () => {
      const driversWithFuelLevy = mockDriverData.filter(
        (d) => d.fuelLevy !== null,
      );
      expect(driversWithFuelLevy).toHaveLength(2);

      // Fuel levy typically applies to Contractors and Subcontractors
      const contractorWithFuelLevy = driversWithFuelLevy.find(
        (d) => d.type === "Contractor",
      );
      const subcontractorWithFuelLevy = driversWithFuelLevy.find(
        (d) => d.type === "Subcontractor",
      );
  ...
  describe("Driver form validation logic", () => {
    ...
    it("should validate break deduction field", () => {
      const validateBreaks = (breaks: number | null) => {
        return (
          breaks === null ||
          (typeof breaks === "number" && breaks >= 0 && breaks <= 60)
        );
      };
  ...
  describe("Driver filtering and search", () => {
    it("should search drivers by multiple criteria", () => {
      ...
      const searchResults = mockDriverData.filter(
        (d) =>
    ...
    it("should filter by rate availability", () => {
      ...
      const driversWithTrayAndCrane = mockDriverData.filter(
        (d) => d.tray !== null && d.crane !== null,
      );
      expect(driversWithTrayAndCrane).toHaveLength(1);
      expect(driversWithTrayAndCrane[0].driver).toBe("John Smith");

      const driversWithSemiAndSemiCrane = mockDriverData.filter(
        (d) => d.semi !== null && d.semiCrane !== null,
      );
    ...
    it("should group drivers by type", () => {
      const groupedByType = mockDriverData.reduce(
        (groups, driver) => {
  ...
  describe("Driver statistics and calculations", () => {
    it("should calculate average rates by type", () => {
      const employees = mockDriverData.filter((d) => d.type === "Employee");
      const avgEmployeeTrayRate =
        employees.reduce((sum, d) => sum + (d.tray || 0), 0) / employees.length;
      expect(avgEmployeeTrayRate).toBe(150);
    });

    it("should identify highest and lowest rates", () => {
      const trayRates = mockDriverData
        .map((d) => d.tray)
    ...
    it("should count drivers by capability", () => {
      ...
      const craneCapableDrivers = mockDriverData.filter(
        (d) => d.crane !== null,
      );
    ...
    it("should validate business rules for driver types", () => {
      ...
      const subcontractorsWithFuelLevy = subcontractors.filter(
        (d) => d.fuelLevy !== null,
      );
      const subcontractorsWithTolls = subcontractors.filter(
        (d) => d.tolls === true,
      );
