...
describe('MultiJobAttachmentUpload', () => {
  ...
  describe('Helper functions - formatJobLabel', () => {
    it('formats job label as DD/MM/YY - Driver - Customer', () => {
      const formatJobLabel = ({ job }: { job: Job }): string => {
        const dateStr =
          job.date.slice(8, 10) +
          '/' +
          job.date.slice(5, 7) +
  ...
  describe('Helper functions - formatFileSize', () => {
    it('formats zero bytes correctly', () => {
      const formatFileSize = ({ bytes }: { bytes: number }): string => {
  ...
  describe('Validation logic', () => {
    ...
    it('attachment types default to "runsheet"', () => {
      ...
      const createJobFile = ({ file }: { file: File }) => ({
      ...
      const jobFile = createJobFile({ file: testFile })
  ...
  describe('File-by-job data structure logic', () => {
    it('groups files by job ID', () => {
      const filesByJob: Record<number, Array<{ id: string; file: File; attachmentType: string }>> = {}
  ...
  describe('Close behaviour tests', () => {
    ...
    it('state is reset when dialog closes', () => {
      ...
      const resetState = () => {
        const clearedFiles: Record<number, Array<{ id: string; file: File; attachmentType: string }>> = {}
    ...
    it('shows failure toast when all jobs fail', () => {
      ...
      if (updatedJobs.length === 0) {
        mockToast({
          title: 'Upload failed',
          description:
            'Failed to upload files to any of the selected jobs',
          variant: 'destructive',
        })
      }
