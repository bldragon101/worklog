...
describe("ChangelogDialog", () => {
  const mockReleases: Release[] = [
    {
      version: "2.0.0",
      date: "2024-02-01",
      features: [{ text: "New dashboard" }, { text: "API v2" }],
      bugFixes: [{ text: "Fix memory leak" }, { text: "Resolve crash issue" }],
      breaking: [{ text: "API endpoints changed" }],
      userNotes: {
    ...
    },
    {
      version: "1.1.0",
      date: "2024-01-15",
      features: [{ text: "Add export feature" }],
      bugFixes: [{ text: "Fix login bug" }],
      breaking: [],
    },
    {
      version: "1.0.0",
  ...
  it("should show Current badge for current version", () => {
    ...
    expect(v110Element.parentElement?.parentElement).toContainElement(
      currentBadge,
    );
  });
  ...
  it("should show Latest badge for newest non-current version", () => {
    ...
    expect(v200Element.parentElement?.parentElement).toContainElement(
      latestBadge,
    );
  });
  ...
  it("should expand first version by default", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Brand new dashboard experience"),
      ).toBeInTheDocument();
    });
  });
