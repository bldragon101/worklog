...
describe("Jobs Page Logic", () => {
  ...
  describe("Job date filtering", () => {
    it("should filter jobs by year", () => {
      const year2024Jobs = mockJobData.filter(
        (j) => getYear(parseISO(j.date)) === 2024,
      );
      expect(year2024Jobs).toHaveLength(2);
    });

    it("should filter jobs by month", () => {
      const januaryJobs = mockJobData.filter(
        (j) => getMonth(parseISO(j.date)) === 0, // January is 0
      );
      expect(januaryJobs).toHaveLength(2);
    });

    it("should filter jobs by day of week", () => {
      const mondayJobs = mockJobData.filter(
        (j) => getDay(parseISO(j.date)) === 1, // Monday is 1
      );
    ...
    it("should filter jobs within a week", () => {
      const weekStart = startOfWeek(parseISO("2024-01-15"), {
        weekStartsOn: 1,
      });
  ...
  describe("Job operations", () => {
    ...
    it("should calculate total charged hours", () => {
      const totalHours = mockJobData.reduce(
        (sum, job) => sum + (job.chargedHours || 0),
        0,
      );
    ...
    it("should calculate total driver charges", () => {
      const totalCharges = mockJobData.reduce(
        (sum, job) => sum + (job.driverCharge || 0),
        0,
      );
      expect(totalCharges).toBe(630.0);
    });
  });
});
